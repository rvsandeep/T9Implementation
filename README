...
On pressing 0,word corresponding to the previous new sequence should be appended to the previous message
and space should be provided.
On pressing * the next word corresponding to the sequence should be provided as an option.

[DONE]
T9 implmentation.
A static Dicitonary is built.
...
How to Use?
Run the GUI.
Use it just like the normal mobile keypad.
Press 0 for space * for other words.

What could go wrong?
If an sequence is provided which is not present in the dictionary and is not the prefix of any other sequence
in the dictionary, then exception is thrown. GUI stops working. Rerun to use.

[TODO] 

1.  Right now im using the frequency count as the number of occurances of the word in the dictionary. It should be
	changed to number of times it is used by the user.
2.	Word Prediction/Completion to be Implemented
3.	Option to add words to the dicitionary (Dynamic Dictionary).
	This will also solve our problem of GUI crashing when sequence is not present and is not prefix of any other words in the dict.
4.	Interface can be improved to provide edit option.
5.	Capital cases can be provided.
